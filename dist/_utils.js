...
var addClassName = (exports.addClassName = function addClassName(path, jsxId) {
  ...
  for (var i = attributes.length - 1, attr; (attr = attributes[i]); i--) {
    ...
    if (t.isJSXSpreadAttribute(attr)) {
      ...
      var attrNameDotClassName = t.memberExpression(
...
var validateExternalExpressions = (exports.validateExternalExpressions = function validateExternalExpressions(
  path
) {
  try {
...
var getJSXStyleInfo = (exports.getJSXStyleInfo = function getJSXStyleInfo(
  expr,
  scope
) {
  var node = expr.node
...
var computeClassNames = (exports.computeClassNames = function computeClassNames(
  styles,
  externalJsxId
) {
  if (styles.length === 0) {
  ...
  var dynamic = t.callExpression(
    ...
    t.memberExpression(
      t.identifier(_constants.STYLE_COMPONENT),
...
var templateLiteralFromPreprocessedCss = (exports.templateLiteralFromPreprocessedCss = function templateLiteralFromPreprocessedCss(
  css,
  expressions
) {
  var quasis = []
...
var makeStyledJsxTag = (exports.makeStyledJsxTag = function makeStyledJsxTag(
  ...
  if (expressions.length > 0) {
    attributes.push(
      t.jSXAttribute(
        t.jSXIdentifier(_constants.STYLE_COMPONENT_DYNAMIC),
...
var processCss = (exports.processCss = function processCss(
  stylesInfo,
  options
) {
  ...
  if (useSourceMaps) {
    ...
    transformedCss = (0, _styleTransform2.default)(
      isGlobal ? '' : getPrefix(dynamic, staticClassName),
      plugins(css, pluginsOptions),
      { splitRules: splitRules, vendorPrefixes: vendorPrefixes }
    )
  }
...
var setStateOptions = (exports.setStateOptions = function setStateOptions(
  state
) {
